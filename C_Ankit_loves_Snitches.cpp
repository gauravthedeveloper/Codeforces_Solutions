...
bool is_snitch(std::string num_str)
{
    int sum_of_digits = 0;
    ...
    if (num_str[len - 1] - '0' == 2 || num_str[len - 1] - '0' == 4 || num_str[len - 1] - '0' == 6 || num_str[len - 1] - '0' == 8 || num_str[len - 1] - '0' == 0)
    {
        return false;
    }
    return true;
}
string findi(string &str, int index, string special, int n)
{
    if (index == n && special.size() > 0)
    {
        if (is_snitch(special))
            return special;
    }

    special += str[index];
    findi(str, index + 1, special, n);
    special = special.substr(0, special.size() - 2);
    findi(str, index + 1, special, n);
